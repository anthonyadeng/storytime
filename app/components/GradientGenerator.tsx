...
export const GradientGenerator: React.FC<GradientGeneratorProps> = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numberOf; i++) {
      const screenYPos = Math.floor(Math.random() * screenHeight);
      const screenXPos = Math.floor(Math.random() * screenWidth);
      const height = Math.floor(Math.random() * 1000 + 250); // Random size between 50 and 150
      const width = Math.floor(Math.random() * 1000 + 250); // Random size between 50 and 150
  ...
  return <div className=' w-full h-full'>{gradients}</div>;
};
